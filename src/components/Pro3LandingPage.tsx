...
export function Pro3LandingPage({ search }: { search: LandingSearch }) {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-background pb-24 text-foreground md:pb-0">
      ...
      <Section>
        <LandingFaq
          eyebrow={PRO3_COPY.faq.eyebrow}
          title={PRO3_COPY.faq.title}
          sub={PRO3_COPY.faq.sub}
          itens={PRO3_COPY.faq.itens}
        />
      </Section>
      ...
      <LandingFooter brand={PRO3_COPY.brand} tagline={PRO3_COPY.footerTagline} zap={zap} />

      <StickyCta
        label={`${PRO3_COPY.hero.cta} · ${precoAtivo}`}
        hint={PRO3_COPY.garantia.curta}
        onClick={() => irParaCheckout(planoAtivo)}
      />
    </main>
  );
